...
and resizes it to the given crop size and computes the optical flow between consecutive
frames


The output are two .npy files, one containing the input data, which consists of
chunks of frames according to the windowsize.
Its dimension is (chunks, windowsize, crop, crop)
...
data_handler  = dp.DataHandler((windowsize, cropsize, cropsize, 3))
...
for n in range(1,6):
    ...
    for i, trial in enumerate(trials):
        ...
        frames = video_handler.get_optical_flow_frames(trial['start'], trial['end'], cropsize)
